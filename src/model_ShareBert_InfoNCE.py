...
class DualEncoderModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        se = outputs.last_hidden_state[:, 0, :]
        output = self.fc(se)
...
def train_model(model, dataloader, optimizer, scheduler, device):
    ...
    for batch in dataloader:
        ...
        i+=1
        # 获取query和doc的表示
        query_embeddings = model(batch['query_input_ids'], batch['query_attention_mask'])
        pos_doc_embeddings = model(batch['pos_doc_input_ids'], batch['pos_doc_attention_mask'])
        batch_size, num_docs, seq_length = batch['neg_doc_input_ids'].shape  # [1,30,512]
        neg_doc_input_ids = batch['neg_doc_input_ids'].view(batch_size * num_docs, seq_length)  # Shape: [30,512,768]
        neg_doc_attention_mask = batch['neg_doc_attention_mask'].view(batch_size * num_docs, seq_length)
        neg_doc_embeddings = model(neg_doc_input_ids, neg_doc_attention_mask)
        # 计算InfoNCE损失
...
if __name__ == "__main__":
   ...
   # data=[]
    ...
    torch.save(model.state_dict(), '../model/shareLawformer_InfoNCE/pytorch_model.bin')
